 ...
 * @author ADMIN
 */
@WebServlet(urlPatterns = {"/Addaction"})
public class Addaction extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
		...
		try 
		{
		    ...
		    PreparedStatement query=con.prepareStatement("insert into Addoptical(gname,brand,shape,size,type,color,image,cost)values ('"+m.get(0)+"','"+m.get(1)+"','"+m.get(2)+"','"+m.get(3)+"','"+m.get(4)+"','"+m.get(5)+"','"+ms.get(0)+"','"+m.get(6)+"')");
                    System.out.println(query);
                    query.executeUpdate();
                    System.out.println(query);
                    out.println("<script>"); 			
                    out.println("alert(\"Add Materials successfully\")");
                    out.println("</script>");
